...
export function createContext(options: Options = {}, root = process.cwd()) {
  ...
  const path = resolve(root, 'src/poto-auto-imports.ts')
  ...
  function generateImports() {
    ...
    unimport.getImports().sort((a, b) => (a.as ?? a.name).localeCompare((b.as ?? b.name))).forEach((i) => {
      ...
      importStr += `  ${name}: { widgetView: ${name}.widgetView, widgetSettingsView: ${name}.widgetSettingsView, defaultSettings: ${name}.defaultSettings },\n`
    })

    const importObj = `export const widgetPlugins = {\n${importStr}}\n`
  ...
  function writeConfigFiles() {
    ...
    if (path) {
      ...
      if (content !== lastImport) {
        lastImport = content
        promises.push(fs.writeFile(path, content, 'utf-8'))
      }
    }
