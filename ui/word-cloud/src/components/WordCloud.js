...
class WordCloud {
  constructor(elements, options) {
    ...
    // console.log(typeof options.weightFactor, options.weightFactor)
    ...
    const getRotateDeg = function getRotateDeg() {
      ...
      if (!settings.randomRotation && settings.customRotations.length > 0) {
        const idx = Math.floor(Math.random() * settings.customRotations.length)
        return settings.customRotations[idx] * Math.PI / 180
      }
    ...
    const drawText = function drawText(gx, gy, info, word, weight, wordColor, distance, theta, rotateDeg, attributes, extraDataArray) {
    ...
    const putWord = function putWord(item) {
      let word, weight, attributes, rotate, wordColor
      if (Array.isArray(item)) {
        word = item[0]
        weight = item[1]
        wordColor = item[2]
      ...
      const tryToPutWordAtPoint = function (gxy) {
        ...
        drawText(gx, gy, info, word, weight, wordColor,
    ...
    this.start = function start() {
      ...
      if (!canvas.getContext || settings.clearCanvas) {
        ...
        let bctx = document.createElement('canvas').getContext('2d', { willReadFrequently: true })
        ...
        let imageData = canvas.getContext('2d', { willReadFrequently: true }).getImageData(0, 0, ngx * g, ngy * g).data
      ...
      i = 0
      const loopingFunction = setTimeout
      const stoppingFunction = clearTimeout
