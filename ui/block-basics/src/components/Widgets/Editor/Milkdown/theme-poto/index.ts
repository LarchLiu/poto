...
import { darkColor, lightColor } from './poto'
...
export const getPoto = (isDarkMode = false) =>
  themeFactory((emotion, manager) => createTheme(isDarkMode)(emotion, manager))

export const potoDark = getPoto(true)
export const potoLight = getPoto(false)

let darkMode = false
if (typeof window !== 'undefined')
  darkMode = Boolean(window.matchMedia?.('(prefers-color-scheme: dark)').matches)

export const poto = getPoto(darkMode)

export { color, darkColor, lightColor } from './poto'
